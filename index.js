...
function customAssign(...objects) {
  ...
  for (let key in mergedObj) {
    if (typeof mergedObj[key] === "number") {
      mergedObj[key] = String(mergedObj[key]);
    }
...
function customObjectFromEntries(entries) {
  ...
  const processedEntries = entries.map(([key, value]) => {
    // Якщо значення є числом, перетворюємо його на рядок
    if (typeof value === "number") {
      value = value.toString();
    }

    // Якщо значення є рядком, замінюємо пробіли на підкреслення
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  Object.freeze(mergedObjects);
  // Перевіряємо, чи об'єкт "заморожено"
  if (Object.isFrozen(mergedObjects)) {
...
function filterObjectsByKey(arr, key) {
  ...
  const objectKey = arr.filter((obj) => obj.hasOwnProperty(key));
...
function checkOwnership(obj1, obj2, key) {
  ...
  if ((obj1.hasOwnProperty(key) && obj2.hasOwnProperty(key)) || obj1 === obj2) {
    return true;
  } else {
    return false;
  }
